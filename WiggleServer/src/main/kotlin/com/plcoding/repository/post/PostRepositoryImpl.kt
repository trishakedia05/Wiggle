...
class PostRepositoryImpl(db: CoroutineDatabase) :
    PostRepository
{
    ...
    override suspend fun getPostByFollows(
        ...
        val userIdsFromFollows = following.find(Following::followingUserId eq ownUserId).toList().map {
            it.followedUserId
        }
        print("\n $ownUserId \n")
        print("\n $userIdsFromFollows \n")

        return posts.find(Post::userId `in` userIdsFromFollows).skip(page * pageSize)
            .limit(pageSize)
    ...
    override suspend fun getPostDetails(userId: String, postId: String): PostResponse? {
        val isLiked = likes.findOne(and(Like::userId eq userId, Like::parentId eq postId )  ) != null
        print("\nisLiked \n $isLiked gb\n")
